...
df.cot$data <- factor(df.cot$data, levels = c("rice.20x", "arab.20x"))
...
p1 <- ggplot() +
  ...
  theme(legend.position = "bottom",
        ...
        legend.text = element_text(size=18, face="italic", family = "serif"),
        legend.key.size = unit(0.35, "cm"),
        legend.margin=margin(-5, 0, 0, 0),
        plot.title = element_text(size=30, hjust = -0.06, face = "bold", family="serif", 
                                  vjust = -0.1),
...
df.cotfb <- ddply(df.cotfb, .(motif, method),
                  transform,
                  pos = cumsum(Pearson_Correlation))
df.cotfb$data <- factor(df.cotfb$data, levels = c("rice.20x", "arab.20x"))
...
p2 <- ggplot() +
  ...
  theme(legend.position = "bottom",
        legend.title = element_blank(),
        legend.text = element_text(size=16, face="italic", family = "serif"),
        legend.key.size = unit(0.35, "cm"),
        legend.margin=margin(-5, 0, 0, 0),
        plot.title = element_text(size=30, hjust = -0.13, face = "bold", family="serif", 
                                  vjust = -0.1),
        strip.background = element_rect(colour="white", fill="white",
                                        size=1, linetype="solid"),
        strip.text.x = element_text(size = 18),
...
df.vmeg <- read.table("tools_to_cmp/eval.tools_cmp.10-50x.arab_rep123.rice2-1_rep2-1.dp0.8.txt", 
                      header = T, 
                      sep = "\t", stringsAsFactors = F)
df.vmeg$coverage <- paste(substr(df.vmeg$coverage, 1, 2), "\U00D7", sep = "")
