...
cg_n_bs_low <- sum(df_cg_a$binname=="low frequency" & df_cg_a$method=="bisulfite")
cg_n_bs_int <- sum(df_cg_a$binname=="intermediate frequency" & df_cg_a$method=="bisulfite")
cg_n_bs_hig <- sum(df_cg_a$binname=="high frequency" & df_cg_a$method=="bisulfite")
df_cg_a$n_exp <- format(cg_n_bs_low, big.mark = ",", scientific = F)
df_cg_a[df_cg_a$binname=="intermediate frequency",]$n_exp <- format(cg_n_bs_int, 
                                                                    big.mark = ",", scientific = F)
df_cg_a[df_cg_a$binname=="high frequency",]$n_exp <- format(cg_n_bs_hig, big.mark = ",", scientific = F)
...
p_cg_a <- ggplot(df_cg_a, aes(x=binname, y=rmet)) + 
...
chg_n_bs_low <- sum(df_chg_a$binname=="low frequency" & df_chg_a$method=="bisulfite")
chg_n_bs_int <- sum(df_chg_a$binname=="intermediate frequency" & df_chg_a$method=="bisulfite")
chg_n_bs_hig <- sum(df_chg_a$binname=="high frequency" & df_chg_a$method=="bisulfite")
df_chg_a$n_exp <- format(chg_n_bs_low, big.mark = ",", scientific = F)
df_chg_a[df_chg_a$binname=="intermediate frequency",]$n_exp <- format(chg_n_bs_int, 
                                                                    big.mark = ",", scientific = F)
df_chg_a[df_chg_a$binname=="high frequency",]$n_exp <- format(chg_n_bs_hig, big.mark = ",", 
                                                              scientific = F)
...
chh_n_bs_low <- sum(df_chh_a$binname=="low frequency" & df_chh_a$method=="bisulfite")
chh_n_bs_int <- sum(df_chh_a$binname=="intermediate frequency" & df_chh_a$method=="bisulfite")
chh_n_bs_hig <- sum(df_chh_a$binname=="high frequency" & df_chh_a$method=="bisulfite")
df_chh_a$n_exp <- format(chh_n_bs_low, big.mark = ",", scientific = F)
df_chh_a[df_chh_a$binname=="intermediate frequency",]$n_exp <- format(chh_n_bs_int, 
                                                                      big.mark = ",", scientific = F)
df_chh_a[df_chh_a$binname=="high frequency",]$n_exp <- format(chh_n_bs_hig, big.mark = ",", 
                                                              scientific = F)
...
ppi= 300
png("tools_to_cmp/comparison_highly_lowly_methylated_sites2.boxplot.arab.raw2.png", 
    width = 39, 
...
svg("tools_to_cmp/comparison_highly_lowly_methylated_sites2.boxplot.arab.raw2.svg",
    width = 39/2.54,
    height = 26/2.54)
grid.arrange(arrangeGrob(p_cg_a, grid.rect(gp=gpar(col="white")), p_chg_a,
                         nrow=1,
