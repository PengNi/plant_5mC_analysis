...
def _read_one_mod_bed_file(freqfile, args):
    # methylbed format
    ...
    with open(bs_file, "r") as rf:
        ...
        for line in rf:
            ...
            if args.contig_prefix is not None:
                ...
                cnt_flag = 1
            if cnt_flag == 1:
                if coverage >= args.covcf_bs:
                    freqinfo[mkey] = rmet
    return freqinfo


def _read_one_mod_bed_file_bs(freqfile, args):
    # methylbed format
    # "chromosome", "pos", "end", "na1", "na2", "strand", "na3", "na4", "na5", "coverage", "rpercent"
    contigset = set(args.contig_names.strip().split(",")) if args.contig_names is not None else None
    freqinfo = {}
...
def _cal_pearson_corr_of_rmet1_and_rmet2(freqinfo1, freqinfo2):
    ...
    return rmet1, rmet2, len(freqinfo1.keys()), len(freqinfo2.keys()), len(keys_inter), corr


def cmp_sitesrmet_of_nano_and_bs(args):
    print("==nano: {}".format(args.modsfile))
    if str(args.modsfile).endswith(".bed"):
        freqinfo_nano = _read_one_mod_bed_file(args.modsfile, args)
    else:
        freqinfo_nano = _read_one_mod_freq_file(args.modsfile, args)
    freqinfo_bs = []
    for bsfile in args.bsfile:
        if str(bsfile).endswith(".bed"):
            freqtmp = _read_one_mod_bed_file_bs(bsfile, args)
        else:
            freqtmp = _read_one_bs_rmet_file(bsfile, args)
        _, _, nanonum, bsnum, sitesnum, corr = _cal_pearson_corr_of_rmet1_and_rmet2(freqinfo_nano, freqtmp)
        print("==bs: {}, sites: inter-{}/nano-{}/bs-{}, corr: {:.4f}".format(bsfile, sitesnum, nanonum,
                                                                             bsnum, corr))
    ...
    rmet_nano, rmet_bs, nanonum, bsnum, sitesnum, corr = _cal_pearson_corr_of_rmet1_and_rmet2(freqinfo_nano,
                                                                                              freqinfo_bs_comb)
    print("==bs: combined, sites: inter-{}/nano-{}/bs-{}, corr: {:.4f}".format(sitesnum, nanonum,
                                                                               bsnum, corr))
...
def main():
    ...
    parser.add_argument("--modsfile", action="store", type=str, required=True,
                        help="50x call_mods_freq/bed file")
    ...
    parser.add_argument("--covcf", type=int, required=False, default=5,
                        help="")
    parser.add_argument("--covcf_bs", type=int, required=False, default=5,
                        help="")
