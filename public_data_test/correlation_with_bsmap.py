...
def read_rmetfile_of_bs(bs_file, contig_prefix, contig_names, cov_cf, is_pval):
    ...
    if is_pval:
        rmet_bs = rmet_bs[rmet_bs["na1"] <= 0.05]
...
def cal_corr_df1_vs_df2(df1, df2):
    ...
    _, _, r_square, _, _ = scipy.stats.linregress(df1_array, df2_array)  # coefficient of determination
...
def correlation_with_bs_rmets(args):
    ...
    for dp2_file in nano_files:
        ...
        if str(dp2_file).endswith("fb_combined.bed"):
            ...
            mean_cov, dp2rmetinfo = read_nanopolish_freqfile(dp2_file, args.contig_prefix, args.contig_names, args.cov_cf)
...
def main():
    ...
    parser.add_argument("--pval", action="store_true", default=False, help="is using pval to filter bs sites "
                                                                           "(in bsmap)")
